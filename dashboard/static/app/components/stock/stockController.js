...
angular.module('dashboard')
    ...
    function($scope, StockService, $rootScope, NgTableParams, FilterService, MonthService)
    {
        ...
        vm.getStockByDistrict = function(startMonth, endMonth, district, vaccine) {
            ...
            var series = [];
            var target_series = [];
            var min = vm.data[0].stock_requirement__minimum;
            var max = vm.data[0].stock_requirement__maximum;

            for (var i = 0; i < vm.data.length ; i++) {
                series.push([MonthService.getMonthName(vm.data[i].month), vm.data[i].consumed]);
                target_series.push([MonthService.getMonthName(vm.data[i].month), vm.data[i].stock_requirement__target]);
            }
